...
async function rag(question: string, limit: number = 5): Promise<void> {
  ...
  console.log(
    `${results.length} chunks trouvés (${results.map((r) => `${(r.similarity * 100).toFixed(0)}%`).join(", ")})`,
  );
  ...
  const context = results
    .map(
      (r, i) =>
        `[Source ${i + 1}: ${r.source_name}/${r.source_type}]\n${r.content}`,
    )
    .join("\n\n---\n\n");
