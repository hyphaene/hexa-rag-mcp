...
import { z } from "zod";
import { getEmbedding, checkOllama, setModel } from "./embedder.js";
import { searchSimilar, searchHybrid, getStats, closePool, setDbModel, } from "./db.js";
import { rerank, checkReranker } from "./reranker.js";
import { generateAnswer, checkGenerator, setLLM, LLM_MODELS, } from "./generator.js";
...
// Tool: search
server.tool("search", "Search the Hexactitude knowledge base using semantic similarity. Searches across knowledge files, scripts, plugins, glossary, front/bff code, and contracts.", {
    ...
    try {
        ...
        const formattedResults = results.slice(0, limit).map((r, i) => {
            ...
            const preview = firstLine.slice(0, 100) + (firstLine.length > 100 ? "..." : "");
...
// Tool: rag
server.tool("rag", "Search and generate a synthesized answer using RAG (Retrieval Augmented Generation). Best for questions that need a comprehensive answer from multiple sources.", {
        ...
        if (!(await checkGenerator())) {
            return {
                content: [
                    {
                        type: "text",
                        text: `Error: LLM ${llm} not available. Run: ollama pull ${LLM_MODELS[llm]}`,
                    },
                ],
            };
        }
...
// Tool: stats
server.tool("stats", "Get statistics about the indexed knowledge base (chunks, files, types, sources)", {}, async () => {
    try {
