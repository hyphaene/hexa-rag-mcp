 ...
 */
export async function getSyncState(filePath) {
    const pool = await getPool();
    const result = await pool.query("SELECT last_mtime, file_hash FROM sync_state WHERE source_path = $1", [filePath]);
    return result.rows[0] || null;
}
/**
 * Update sync state for a file.
 */
export async function updateSyncState(filePath, mtime, hash) {
    const pool = await getPool();
    await pool.query(`INSERT INTO sync_state (source_path, last_mtime, file_hash)
     VALUES ($1, $2, $3)
     ON CONFLICT (source_path)
     DO UPDATE SET last_mtime = $2, file_hash = $3`, [filePath, mtime, hash]);
}
/**
