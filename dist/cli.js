...
import { loadConfig, getConfig, getConfigPath, CONFIG_TEMPLATE, getEmbeddingModel, EMBEDDING_MODELS, LLM_MODELS, getRerankerModel, getGlobalConfigDir, getGlobalConfigPath, } from "./config.js";
import { getLLM } from "./generator.js";
import { getPool, closePool, getStats } from "./db.js";
const VERSION = "0.1.0";
...
// Commands
// ============================================================================
async function cmdInit(interactive, global) {
    ...
    if (interactive) {
        ...
        if (dbPort)
            config.database.port = parseInt(dbPort, 10);
        ...
        const embedding = await prompt(`  Embedding model (${embeddingModels}) [${config.models.embedding}]: `);
        if (embedding &&
            (embedding === "nomic" || embedding === "e5" || embedding === "bge")) {
            config.models.embedding = embedding;
        }
        const llmModels = Object.keys(LLM_MODELS).join(", ");
        const llm = await prompt(`  LLM for RAG (${llmModels}) [${config.models.llm}]: `);
        ...
        while (true) {
            ...
            const typeOptions = "knowledge, code, script, plugin, glossary, contract, doc";
            const type = await prompt(`    Type (${typeOptions}) [code]: `);
            const patterns = await prompt("    Patterns (comma-separated) [**/*.ts]: ");
            const exclude = await prompt("    Exclude (comma-separated) [**/node_modules/**]: ");
            sources.push({
                name,
                path,
                type: (type || "code"),
...
async function cmdDoctor() {
    ...
    try {
        ...
        console.log(`⚠️  No config file found (will use defaults). Run: hexa-vector init --global`);
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            const hasEmbedding = availableModels.some((m) => m === embModel.ollamaModel || m.startsWith(embModel.ollamaModel));
            if (hasEmbedding) {
            ...
            const hasReranker = availableModels.some((m) => m === rerankerModel || m.startsWith(rerankerModel));
            if (hasReranker) {
            ...
            const hasLLM = availableModels.some((m) => m === llmModel || m.startsWith(llmModel.split(":")[0]));
            if (hasLLM) {
...
async function cmdIngest(sourceFilter) {
    loadConfig();
    const config = getConfig();
    if (config.sources.length === 0) {
